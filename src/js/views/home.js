						...
						{store.people && (
							store.people.map((personaje, index) => (
								<div className='swiper-slide-paginas ps-4 pt-3 personajes fade-in'>
									<CardComponent key={index} title={personaje.name} paginaIndividual={`/info_personaje/${personaje.uid}/${personaje.name}`} info={''} favourite={() => actions.addFavourite(personaje.name)} img={
						...
						{store.vehicle && (
							store.vehicle.map((vehiculo, index) => (
								<div className='swiper-slide-paginas vehicles ps-4 pt-3'>
									<CardComponent key={index} title={vehiculo.name} paginaIndividual={`/info_vehiculo/${vehiculo.uid}/${vehiculo.name}`} info={''} favourite={() => actions.addFavourite(vehiculo.name)} img={
						...
						{store.planets && (
							store.planets.map((planeta, index) => (
								<div className='swiper-slide-paginas planets ps-4 pt-3 fade-in'>
									<CardComponent key={index} title={planeta.name}  favourite={() => actions.addFavourite(planeta.name)} paginaIndividual={`/info_planeta/${planeta.uid}/${planeta.name}`} info={''} img={
